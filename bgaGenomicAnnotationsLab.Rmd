...
geneScores = data_frame(gene_symbol = names(annoOutput$Scores), 
  ...
  mutate(gene_symbol = factor(gene_symbol, levels = gene_symbol)) %>% # mutate adds new variables or overwrites existing ones
...
p = geneScores %>% 
  ...
  geom_col(aes(color = inTrainSet, # Add a bar for each gene
               fill = inTrainSet)) + # Use an aesthetic mapping of group membership to border color and fill color.
