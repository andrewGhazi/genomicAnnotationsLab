...
Add the argument `lib.loc = '/home/student20/R/x86_64-redhat-linux-gnu-library/3.2'` to your calls to `library()` if you're running this on Sphere.
...
geneScores = data_frame(gene_symbol = names(annoOutput$Scores), #create a data_frame with the scores and symbols
...
enrichedAnnotations = annoOutput$Enriched.Annotations %>% #create a data_frame from the enriched annotation information
...
For the homework, you'll repeat the lab with a different annotation source and gene set. Show your code and appropriate outputs in a document. If you'd like to learn how to author analysis documents (like this lab), RMarkdown would be a good way to do this.

1. Choose an alternate annotation source. There's another dataset called `miRNA` included with the package that shows how >30,000 genes are annotated in miRBase. You can use this but you can earn a bonus point if you compile your own annotation matrix from a source that's personally interesting to you :).  
